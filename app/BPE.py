...
class BytePairEncoder:
    ...
    def train(self, corpus, vocab_size=None, num_merges=None):
        ...
        for merge_rank in range(remaining_merges):
            ...
            if frequency < 2:
                print(
                    f"No pairs occur more than once. Stopping after {merge_rank} merges."
                )
                break

            self.merges.append(most_frequent)
            self.merge_ranks[most_frequent] = merge_rank
            ...
            if merge_rank % 100 == 0 and merge_rank > 0:
                print(
                    f"Completed {merge_rank} merges. Most recent: {most_frequent} -> {new_token} (freq: {frequency})"
                )

            tokenized_corpus = self._merge_corpus(
                tokenized_corpus, most_frequent, new_token
            )
    ...
    def encode(self, text):
        ...
        for i, word in enumerate(words):
            ...
            if (
                i > 0
                and self.vocab.get(" ") is not None
                and word not in self.punctuation_no_space
            ):
    ...
    def save(self, filepath):
        ...
        model_data = {"vocab": self.vocab, "merges": self.merges}
    ...
    def load(self, filepath):
        ...
        self.vocab = {
            str(k): int(v) if isinstance(v, str) and v.isdigit() else v
        ...
        print(
            f"Loaded tokenizer with {len(self.vocab)} tokens and {len(self.merges)} merges."
        )
        return self
